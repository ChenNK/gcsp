...
public abstract class WebUtil {
    ...
    public static String doPostAmazon(String url, Map<String, String> params, String contentMD5, String contentStr) throws IOException {
        String charset = DEFAULT_CHARSET;
        String ctype = "application/x-www-form-urlencoded;charset=" + charset;
        String query = buildQuery(params, charset);
        HttpURLConnection conn = null;
        ...
        try {
            try {
                ...
                map.put("Host", "mws.amazonservices.com");
                map.put("Transfer-Encoding", "chunked");

                conn = getConnection(new URL(url + "?" + query), "POST", ctype, map);
                conn.setConnectTimeout(CONNECT_TIMEOUT);
                conn.setReadTimeout(READ_TIMEOUT);
            } catch (IOException e) {
                throw e;
    ...
    public static String doPostSuning(String url, Map<String, String> headerMap, String contentStr) throws IOException {
        String charset = DEFAULT_CHARSET;
    ...
    public static String doPostIms(String url, String contentStr) throws IOException {
        String charset = DEFAULT_CHARSET;
        String ctype = "application/x-www-form-urlencoded;charset=" + charset;
        HttpURLConnection conn = null;
        OutputStream out = null;
        ...
        try {
            try {
                ...
                conn = getConnection(new URL(url), "POST", ctype, map);
                conn.setConnectTimeout(CONNECT_TIMEOUT);
                conn.setReadTimeout(READ_TIMEOUT);
            } catch (IOException e) {
                throw e;
            ...
            try {
                out = new DataOutputStream(conn.getOutputStream());
                out.write(contentStr.getBytes(charset));
                rsp = getResponseAsString(conn);
            } catch (IOException e) {
                throw e;
            }
